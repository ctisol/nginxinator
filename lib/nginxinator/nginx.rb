...
namespace :nginx do
  ...
  task :setup => :ensure_setup do
    ...
    on "#{settings.ssh_user}@#{settings.domain}" do
      ...
      settings.config_files.each do |config_file|
        if Nginxinator.config_file_differs?(settings, settings.local_templates_path, settings.external_conf_path, config_file)
      ...
      settings.sites_enabled.each do |config_file|
        if Nginxinator.config_file_differs?(settings, settings.local_site_templates_path, settings.external_sites_enabled_path, config_file)
          warn "Config file #{config_file} on #{settings.domain} is being updated."
      ...
      unless Nginxinator.container_exists?(settings.container_name)
        Rake::Task['nginx:create_container'].invoke
      else
        unless Nginxinator.container_is_running?(settings.container_name)
  ...
  task :status => :ensure_setup do |t, args|
    ...
    on "#{settings.ssh_user}@#{settings.domain}" do
      info ""
      if Nginxinator.container_exists?(settings.container_name)
        info "#{settings.container_name} exists on #{settings.domain}"
        info ""
        if Nginxinator.container_is_running?(settings.container_name)
  ...
  task :create_container => :ensure_setup do |t, args|
    ...
    on "#{settings.ssh_user}@#{settings.domain}" do
      ...
      sleep 2
      fatal Nginxinator.stay_running_message(settings) and raise unless Nginxinator.container_is_running?(settings.container_name)
    end
  end
  ...
  task :install_config_file, [:template_path, :config_path, :config_file] => :ensure_setup do |t, args|
    ...
    on "#{settings.ssh_user}@#{settings.domain}" do
      as 'root' do
        ...
        generated_config_file = Nginxinator.generate_config_file(settings, "#{args.template_path}/#{args.config_file}.erb")
